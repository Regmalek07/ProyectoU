// Función para cargar los productos desde el archivo JSON en el servidor local
function cargarProductos() {
    return new Promise((resolve, reject) => {
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "http://localhost:3000/productos", true); // Ruta al servidor local
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    var data = JSON.parse(xhr.responseText);
                    resolve(data.productos); // Accede al array de productos dentro del JSON
                } else {
